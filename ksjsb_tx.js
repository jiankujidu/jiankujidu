//建议一天跑一次：6 6 * * *
const $ = new Env('快手极速版-提现');
let res,
    ksjsbCookie = $.isNode() ? (process.env.ksjsb_data ? process.env.ksjsb_data : "") : ($.getdata('ksjsb_data') ? $.getdata('ksjsb_data') : "")//快手ck
Users = [],
...
class ksUser {
    ...
    async accountOverview() {
        ...
        if (_0xa69994.result == 1) {
            ...
            // (await $.wait(200), await this.changeExchangeType(0));
        } else {
            console.log(
    ...
    async changeExchangeType(_0x1bd22f) {
        ...
        if (_0x4df55c.result == 1) {
            ...
            // await $.wait(200), await this.exchangeCoin()
        } else {
        console.log(
            '账号[' + this.name + ']兑换方式更改失败：' + _0x4df55c.error_msg
            )
