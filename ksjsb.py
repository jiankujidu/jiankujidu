...
def sign(can_cookie,name):
    ...
    try:
        if result_code==1:
            print("账号[" + name + "]签到任务:" + str(json_arr['data']['toast']))
        else:
            print("账号[" + name + "]签到任务:" + json_arr['error_msg'])
    except TypeError as reason:
        print("查询签到出错啦" + str(reason) + str_result)
...
def zccx(can_cookie,name):
    ...
    try:
        if result_code==1:
            print("账号[" + name + "]资产查询:余额:" + json_arr['data']['totalCash'] + ",金币:" + json_arr['data']['totalCoin'])
        else:
            print("账号[" + name + "]资产查询:查询失败")
    except TypeError as reason:
        print("查询签到出错啦" + str(reason) + str_result)
...
def setShare(can_cookie, name):
    ...
    try:
        if json_arr['result'] == 1:
            ...
            json_arr = json.loads(str_result)
            if json_arr['result'] == 1:
                print("账号[" + name + "]" + "分享任务:" + str(json_arr['data']['amount']))
            else:
                print("账号[" + name + "]" + "分享任务:今日已分享")
        else:
